...
prep_mi_visit <- function(data, spec) {
  ...
  # Visit filter — only apply if visits_model is non-empty
...
prep_obs_dummy_visit <- function(data, spec) {
  ana_raw <- .prep_base(data, spec)

  # Baseline records: subjects with non-missing baseline
  # Used to build the subject list and carry forward baseline info
  base_records <- ana_raw |>
  ...
  # Post-baseline observed records
  obs_records <- ana_raw |>
    dplyr::filter(!is.na(VISITN), VISIT != "Baseline", VISIT != "EOS") |>
    dplyr::select(USUBJID,
                  dplyr::any_of(c("VISIT", "VISITN", "RESP", "DTYPE", "PARAM")))

  # Build dummy visit grid: subjects × scheduled visits
  dummy_visits <- spec$dummy_visits
  dummy_visit_var  <- spec$dummy_visit_var  %||% "AVISIT"
  dummy_visitn_var <- spec$dummy_visitn_var %||% "AVISITN"

  if (length(dummy_visits) == 0)
    dummy_visits <- spec$visits_model

  # Get actual VISITN values from data
  ...
  if (has_param) {
    param_tbl  <- tibble::tibble(PARAM = spec$param_values)
  ...
  # Left-join observed RESP values
