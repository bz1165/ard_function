...
#               Produces compact ARD only (no RTF/TrakData bind in Phase 1)
# PACKAGES    : dplyr, tidyr, purrr, mmrm, emmeans
# PIPE NOTE   : Uses |> (base R pipe, R >= 4.1). No magrittr %>% needed.
#               All { . } dot-placeholder blocks removed for |> compatibility.
# =============================================================================
...
prepare_mmrm_datasets <- function(data, spec) {
  ...
  # NOTE: { d <- . } dot-placeholder only works with %>% not |>
  # Broken into two steps to stay compatible with base R pipe
  dtype_ok <- spec$n_obs_dtype_values %||% c("", "TP")

  obs_n_pre <- ana |>
    dplyr::filter(VISIT %in% spec$visits_display, !is.na(RESP))

  if (!all(is.na(obs_n_pre$DTYPE))) {
...
# =============================================================================
# SECTION 9: TRAKDATA / PDT ADAPTER  (Phase 2 — connect to company pipeline)
#
# HOW SAFETY FUNCTIONS WORK (from funcs_ars.R):
#   create_ard_freq() / create_ard_summary() both call return_ard() at the end.
#   return_ard() does two things when idkey != "":
#     1. Reads TrakData.csv to get title / footnote / shell file path
#     2. Writes an _ard.rds file next to the RTF
#   The ARD gets columns: trakdata, idkey, then all the grp/operation columns.
#
# SAFETY DOES NOT HAVE A SPEC OBJECT.
#   create_ard_freq() / create_ard_summary() are parameter-driven:
#   the user passes grp_var, subj_grp_var, operation, idkey directly
#   as function arguments. There is no list()-based spec.
#   Your efficacy spec approach is deliberately different and more structured,
#   which is fine — the adapter below bridges the two worlds.
#
# WHAT THIS ADAPTER DOES:
#   Takes run_mmrm_ard() output + idkey + trakdata path,
#   adds the trakdata/idkey columns the company pipeline expects,
#   optionally calls return_ard() so the downstream RTF / metadata
#   flow works exactly like safety outputs.
# =============================================================================

#' Attach TrakData metadata to a run_mmrm_ard() result
#'
#' Phase 2 function — only needed when connecting to the company RTF /
#' metadata pipeline. Phase 1 (ARD only) does not require this.
#'
#' Mirrors what return_ard() does in funcs_ars.R:
#'   - adds trakdata and idkey columns to ard
#'   - calls create_report2() to get shell / title / footnote
#'   - writes _ard.rds next to the RTF output
#'
#' @param mmrm_result  list — output of run_mmrm_ard()
#' @param idkey        character — PDT idkey (e.g. "PTL0035")
#' @param trakdata     character — TrakData filename (e.g. "TrakData.csv")
#'
#' @return list with same structure as create_ard_freq() output:
#'   $ard, $subj_N, $shell, $title, $footnote
attach_trakdata <- function(mmrm_result,
                            idkey    = "",
                            trakdata = "TrakData.csv") {

  idkey <- stringr::str_trim(idkey)

  # Add trakdata + idkey columns — same as return_ard() in funcs_ars.R
  ard_out <- mmrm_result$ard |>
    dplyr::mutate(trakdata = trakdata,
                  idkey    = idkey) |>
    dplyr::relocate(trakdata, idkey)

  # subj_N naming: safety uses subj_N (capital N); keep consistent
  subj_N <- mmrm_result$subj_n

  if (idkey != "") {
    # create_report2() is defined in funcs_general.R / funcs_ars.R
    # It reads TrakData.csv and returns shell / title / footnote
    report   <- create_report2(idkey, trakdata = trakdata)
    shell    <- report$shell
    title    <- report$title
    footnote <- report$footnote

    # Write _ard.rds — mirrors return_ard() behaviour
    rds_path <- paste0(dirname(shell$file_path), "/", idkey, "_ard.rds")
    if (!interactive() ||
        (!is.null(st$interactive_write_ard) && st$interactive_write_ard)) {
      readr::write_rds(ard_out, rds_path)
      message("ARD written to: ", rds_path)
    }
  ...
  # No idkey: return same structure as create_ard_*() with idkey=""
...
#
# After sourcing both funcs_general.R (company) and create_ard_mmrm.R (yours):
#
#   source(file.path(st$util, "funcs_general.R"))  # company — has create_report2()
#   source("create_ard_mmrm.R")                    # yours
#
#   adfacmi <- haven::read_sas(file.path(st$analysis, "adfacmi.sas7bdat"))
#
#   # Phase 1: ARD only (no TrakData needed)
#   res <- run_mmrm_ard(adfacmi, spec_facit_14_2_4_1)
#   res$ard
#
#   # Phase 2: attach TrakData (same idkey as SAS PTL0035)
#   res_std <- attach_trakdata(res, idkey = "PTL0035", trakdata = "TrakData.csv")
#   res_std$title
#   res_std$footnote
#   res_std$shell
#
# The returned res_std has the same structure as create_ard_freq() output,
# so any existing post-processing / RTF generation code that expects
# $ard / $subj_N / $shell / $title / $footnote will work unchanged.
# =============================================================================
