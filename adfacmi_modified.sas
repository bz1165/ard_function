...
/* [CHANGE 1] Include dummy visits from source dataset: add "or aval=." condition */
/* [CHANGE 2] Keep EST/ICE/IMPREA variables from updated ADFACIT */
data dat_part;
      ...
      keep  usubjid trt01p trt01pn avisit _avisit avisitn paramcd param base chg aval 
		...
		EST01STP ICE01F IMPREA01 EST02STP ICE02F IMPREA02; /*[CHANGE] Keep EST/ICE/IMPREA variables*/
...
data dat_ana;
	set dat;
	where trt01pn NE .; /* [CHANGE] Use trt01pn instead of trt01p for consistency */
	proc sort;
		by paramcd param usubjid;
run;

/* [CHANGE 3] REMOVED the entire full visit grid creation section:
   - No longer need to create full_visit table
   - No longer need to re-merge baseline variables via proc sql
   - No longer need to merge TR01SDT from adsl
   - No longer need to merge planned visits from SV
   - No longer need to impute dates and flags for missing visits
   All of this is now pre-done in the source dataset ADFACIT
*/
...
data dat_ana4;
	set dat_ana;  /* [CHANGE] Read from dat_ana directly instead of dat_ana1_5 */
	where avisit ^= "Baseline"; 
	AVISITN=input(AVISIT,avisit.);
run;

/* Keep baseline for later merge */
data dat_ana_base;
	set dat_ana;
	where avisit = "Baseline"; 
run;
...
/* [CHANGE 4] Use EST/IMPREA variables instead of ANL05FL/ANL06FL/AEFLAG */
...
data dat_ana_mi_supp;
	...
	ACAT1N=2;
	ana_flag = "supp";
	IMPREAS=IMPREA02; /* [CHANGE] Add IMPREAS variable */
	if systrcrf="" then systrcrf="N";
	/* Administrative missing / Treatment policy: use ALMCF placeholder */
...
%macro facit_mi(endpoint=);
	...
	/* [CHANGE] Add AVISIT derivation to DataFull before merge */
...
data adfacmi_comb1;
	...
	by USUBJID;	
	drop ANL05FL ANL06FL ANL07FL AEFLAG; /* [CHANGE] Drop flags, will re-derive in final step */
run;


/* [CHANGE 5] Re-merge EST/ICE variables from source, similar to aduprmi pattern */
proc sql;
...
data adfacmi_comb1;
	...
	analysis.adfacit(where=(AVISIT="Baseline" and paramcd='FACIT'));
	by USUBJID;	
	drop ANL05FL ANL06FL ANL07FL AEFLAG; /* Drop flags, will re-derive in final step */
run;
...
/* [CHANGE 6] Update final dataset: re-derive ANL05/ANL06/ANL07/AEFLAG from EST variables; add IMPREAS */
data adfacmi_final;
	length PROEVFL $1.;
	...
	/* [CHANGE] Re-derive ANL05FL/ANL06FL/ANL07FL/AEFLAG based on EST variables */
	...
	/* [CHANGE] Derive IMPREAS based on ACAT1 */
	if dtype ne '' then do;
